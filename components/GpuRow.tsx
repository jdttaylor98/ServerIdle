...
import { GpuTier, getGpuCost, getGpuOutput, getGpuRpOutput } from '../engine/gpus';
...
export function GpuRow({ tier }: Props) {
  const credits = useGameStore((state) => state.credits);
  const owned = useGameStore((state) => state.gpus[tier.id] ?? 0);
  const buyGpu = useGameStore((state) => state.buyGpu);
  const sellGpu = useGameStore((state) => state.sellGpu);
  ...
  const totalOutput = getGpuOutput(tier, owned);
  const totalRp = getGpuRpOutput(tier, owned);
  const canAfford = credits >= cost;
  const refund = owned > 0 ? Math.floor(getGpuCost(tier, owned - 1) * 0.5) : 0;

  const isBuildingThis =
    activeBuild?.kind === 'gpu' && activeBuild?.id === tier.id;
    ...
    <View style={styles.row}>
      <View style={styles.info}>
        ...
        <Text style={styles.output}>
          +{tier.baseOutput.toLocaleString()} cr/sec each
        </Text>
        <Text style={styles.rpOutput}>
          +{tier.rpOutput} RP/sec each
        </Text>
        {owned > 0 && (
          <Text style={styles.totalLine}>
            Total: {totalOutput.toLocaleString()} cr/sec · {totalRp.toFixed(1)} RP/sec
          </Text>
        )}
        <Text style={styles.consumption}>
          {tier.powerDraw.toLocaleString()}W · {tier.heatOutput.toLocaleString()} BTU ·{' '}
          {Math.round(tier.buildTimeSeconds / 60)}min build
        </Text>
  ...
  return (
    <View style={styles.buildBox}>
      <View style={styles.buildHeader}>
        <Text style={styles.buildLabel}>FABRICATING…</Text>
